...
## 阶段 21.3D：评级证据展示与人工复核体验

作品详情页的价值评级区域已整理为面向运营复核的工作台：

- 当前采用评级卡只展示已人工采用的 OpenAI rating run。没有采用结果时显示“当前作品尚未采用 OpenAI 价值评级”。
- 待采用评级建议卡展示最新成功但未采用的 OpenAI run，并对比当前评级与新建议的评级、分数和置信度。
- invalid / failed run 会显示运营可读中文原因，并明确当前采用评级不会被覆盖。
- OpenAI Rating Runs 历史列表默认只展示最近 10 条，legacy rules 不混入 OpenAI run 历史。

### 证据分区展示

页面会展示 21.3C 生成的证据分区：

- `acceptedEvidence`：以绿色区域展示，包含来源、来源等级、证据类型、主张、正负向影响、权重和采信原因。高权重证据会突出显示。
- `uncertainEvidence`：默认折叠并弱化展示，只能解释置信度或风险，不能作为核心评级依据。
- `rejectedEvidence`：默认折叠，说明无关、重名、来源不可靠或不应参与评级的结果，不得进入关键证据。
- `missingEvidence`：明确展示“证据缺失不等于负面表现”。`shouldPenalize=false` 的内容标记为“不扣分”。

`evidenceTags` 会单独展示官方/首发平台、可信三方平台、有声平台、社媒热度、IP 改编和作者影响力等标签。每个 true 标签必须能追溯到 `acceptedEvidence`。没有明确 IP 证据时，页面显示“未发现可确认 IP 改编证据。”本地 `preliminarySignals` 不会显示为已确认标签。

### 人工补充证据

运营人员可以在作品详情页新增或删除 `WorkRatingSupplement`。补充证据支持首发/官方平台、三方阅读平台、有声平台、广播剧平台、社媒热度、影视/IP 证据、榜单/销量、作者影响力、运营备注和其他类型。重新评级时，OpenAI 会读取当前全部补充证据并生成新的待采用评级建议。

补充证据要求标题和内容不能为空；证据链接如果填写，必须是 `http` 或 `https` URL。删除补充证据前会二次确认。
