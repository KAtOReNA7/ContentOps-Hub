...
## 阶段 18 多书名测试结果回流与运营复盘闭环

更新时间：2026-05-30

- 新增 WorkExperimentResult / WorkExperimentReview 模型，用于保存多书名测试结果和复盘结论。
- 新增 `/experiments/import` 测试结果导入页面，支持 Excel / CSV 手动导入。
- 新增 `/api/experiments/import`，支持逐行导入，单行失败不影响整批。
- 导入优先使用业务作品 ID externalId 匹配 Work，匹配不到时记录失败原因。
- 新增本地规则复盘逻辑，计算 CTR、转化率、完播率和收入变化。
- 作品详情页新增“测试结果 / 复盘”区域，展示对照组、胜出版本、指标变化、推荐动作、结论和风险。
- 复盘结果不会自动覆盖最终采用结果，用户点击确认后才会更新 finalTitle / finalIntro / finalCoverUrl。
- Excel 导出增加复盘字段：实验名称、对照组、胜出版本、CTR/转化率/完播率/收入变化、推荐动作、复盘结论、置信度和风险提示。
- 首页运营看板增加测试结果相关统计。
- 本阶段不接平台后台自动抓数，不做复杂显著性检验，不默认调用 OpenAI。

## 阶段 17.2：上传文件填写规则与导入校验增强

更新时间：2026-05-30

- 作品导入页已增加“上传文件填写规则”和官方模板下载入口。
- 必填字段收敛为书名；作品 ID、作者、简介、分类作为强烈建议字段。
- 已兼容中文和英文常见表头别名。
- 已增加文件可读性、表头、空行、播放量、点击率、完播率和封面 URL 校验。
- 作品 ID 重复和书名 + 作者重复会在预览中提示，导入时重复作品会跳过。
- 导入结果会展示总行数、成功创建数、失败或重复跳过数、空行跳过数和失败原因。
- 已更新 sample/input-template.xlsx 官方模板。
- 作品清单导入与多书名测试结果导入继续使用独立入口和独立模板。
- 本阶段未修改 Prisma schema，未新增依赖。

## 阶段 18.1：多书名测试结果导入填写规则

更新时间：2026-05-30

- 多书名测试结果导入页已增加独立填写规则，不与作品基础信息导入混用。
- 已增加官方模板下载：sample/experiment-results-template.xlsx。
- 已支持实验名称、作品 ID、书名、作者、组别、实验组名称、测试书名、测试简介、封面地址、曝光量、点击量、点击率、播放量、转化量、转化率、完播率、收入、测试日期和备注的常见中英文别名。
- 已增加组别、作品匹配字段、数量、比例、收入、日期范围和封面 URL 校验。
- 缺少对照组或实验组时允许导入，但暂无法生成完整复盘。
- 导入结果摘要展示成功数、失败数、空行跳过数、匹配作品数、未匹配作品数、可生成复盘数和暂无法复盘数。
- 本阶段未修改 Prisma schema，未新增依赖，未调用 OpenAI。

## 阶段 19：运营效果回流与评分校准

更新时间：2026-05-30

- 新增 WorkFeedbackInsight 模型，保存实验前评级、多书名建议、封面策略和真实测试校准结果。
- 新增本地规则效果洞察服务，不训练模型，不调用 OpenAI。
- 作品详情页新增“效果回流 / 评分校准”区域，可生成或刷新洞察。
- 阶段 18 复盘区域增加“生成效果回流洞察”入口。
- 首页运营看板增加已生成洞察、评级基本准确和数据不足统计。
- Excel 导出增加实际结果判断、评级准确性、书名策略效果、封面策略效果、关键提升指标、策略标签、摘要和风险提示。
- 洞察只用于运营参考，不自动修改评级规则，不自动覆盖最终采用结果。

## 阶段 19.1：项目体验评估与界面清理

更新时间：2026-05-30

- 首页运营看板按处理进度、测试复盘、效果洞察重新分组，并补充建议动作提示。
- 作品详情页新增处理摘要和页内锚点导航，识别候选默认只展示前 5 条，长文本支持折叠查看。
- 作品列表批量操作默认折叠，简介压缩为单行，选中作品数量更醒目。
- 批量任务中心按任务状态显示进度条颜色，单条任务结果摘要默认折叠。
- 导入页补充上传前检查清单和推荐表头复制入口。
