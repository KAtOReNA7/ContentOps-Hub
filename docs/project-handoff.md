...
建议阶段 14 做“筛选导出 / Excel + ZIP 交付包”，不要立刻进入真实搜索 API 或批量图片生成。
...
> 当前 MVP 已经形成“导入 -> 识别 -> 评级 -> 生成建议 -> 封面处理 -> 导出”的基本闭环。默认使用 Mock / 本地规则，OpenAI 文本和 ChatGPT Image2 图片能力均为用户主动触发，不会默认产生外部成本。下一步建议优先补齐人工审核状态流和最终采用结果管理，再做导出交付包增强。
