...
type WorkExportButtonProps = {
  workId: string;
};

export function WorkExportButton({ workId }: WorkExportButtonProps) {
  ...
  async function exportWork() {
    ...
    try {
      await downloadExcel(`/api/export/works/${workId}`, `work-export-${workId}.xlsx`);
    } catch (caughtError) {
